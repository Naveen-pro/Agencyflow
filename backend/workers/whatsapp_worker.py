...
async def whatsapp_campaign_worker(
    campaign_id: UUID,
    agency_id: UUID,
    db: AsyncSession,
    redis_client=None,
    ...
    for contact in contacts:
        ...
        result = await send_whatsapp(phone, personalized)

        if result["success"]:
            log = DeliveryLog(
                campaign_id=campaign_id, contact_name=name, contact_value=phone,
                status="delivered", provider_msg_id=result.get("message_id"),
    ...
    if usage:
        await db.execute(
            ...
            .values(wa_used=UsageLimit.wa_used + total_sent)
        )
    await db.commit()

    if redis_client:
        event = json.dumps({"type": "complete", "stats": {"sent": total_sent, "failed": total_failed}})
        try:
            await redis_client.publish(f"campaign:{campaign_id}", event)
        except Exception:
            pass
